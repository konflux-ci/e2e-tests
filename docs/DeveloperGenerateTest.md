...
or parse Ginkgo Test Files that have large amounts of code, like [contract.go](../tests/enterprise-contract/contract.go), and generate spec outlines that looks like this.

```
EnterpriseContractSuiteDescribe: Conforma E2E tests @ec
  Context: infrastructure is running @pipeline
    It: makes sure the Tekton Chains public key is available
  Context: test creating and signing an image and task @pipeline
    It: verifies the Conforma policy is met
