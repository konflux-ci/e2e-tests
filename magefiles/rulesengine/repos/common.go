...
func GetPairedCommitSha(repoForPairing string, rctx *rulesengine.RuleCtx) string {
	...
	if err := sendHttpRequestAndParseResponse(url, "GET", &pullRequests); err != nil {
		klog.Infof("cannot determine %s Github branches for author %s: %v. will stick with the redhat-appstudio/%s main branch for running tests", repoForPairing, rctx.PrRemoteName, err, repoForPairing)
		return ""
	}
