...
var _ = framework.ReleasePipelinesSuiteDescribe("e2e tests for rh-advisories pipeline", ginkgo.Pending, ginkgo.Label("release-pipelines", "rh-advisories"), func() {
	...
	ginkgo.Describe("Rh-advisories happy path", ginkgo.Label("rhAdvisories"), func() {
		...
		ginkgo.AfterAll(func() {
			...
			gomega.Expect(devFw.AsKubeDeveloper.HasController.DeleteApplication(advsApplicationName, devNamespace, false)).NotTo(gomega.HaveOccurred())
			gomega.Expect(managedFw.AsKubeDeveloper.TektonController.DeleteEnterpriseContractPolicy(advsEnterpriseContractPolicyName, managedNamespace, false)).NotTo(gomega.HaveOccurred())
			gomega.Expect(managedFw.AsKubeDeveloper.ReleaseController.DeleteReleasePlanAdmission(advsReleasePlanAdmissionName, managedNamespace, false)).NotTo(gomega.HaveOccurred())
		})
		...
		var _ = ginkgo.Describe("Post-release verification", func() {
			...
			ginkgo.It("verifies the advs release pipelinerun is running and succeeds", func() {
				gomega.Eventually(func() error {
