...
ERRORS = {
    ...
    ("Couldnt get pipeline because ETCD timeouted", r"Release pipeline run failed: PipelineRun .* in namespace .* failed: .* Reason:CouldntGetPipeline Message:Error retrieving pipeline for pipelinerun .*: resolver failed to get Pipeline : error requesting remote resource: error updating resource request .* with data: etcdserver: request timed out"),
    ("Couldnt get pipeline because ETCD timeouted", r"Build Pipeline Run failed run: PipelineRun for component .* in namespace .* failed: .* Reason:CouldntGetPipeline Message:Error retrieving pipeline for pipelinerun .*: resolver failed to get Pipeline : error requesting remote resource: etcdserver: request timed out"),
    ("Couldnt get task via bundles resolver because can not validate referenced object", r"Build Pipeline Run failed run: PipelineRun for component [^ ]+ in namespace [^ ]+ failed: .* Reason:CouldntGetTask Message:Pipeline [^ ]+ can't be Run; it contains Tasks that don't exist: Couldn't retrieve Task .resolver type bundles.*name = .*: retryable error validating referenced object [^ ]+: Internal error occurred: failed calling webhook .webhook.pipeline.tekton.dev.: failed to call webhook: Post .https://tekton-pipelines-webhook.openshift-pipelines.svc:443/[^ ]+.: EOF"),
    ...
    ("Couldnt get task via bundles resolver because ETCD timeouted", r"Build Pipeline Run failed run: PipelineRun for component .* in namespace .* failed: .* Reason:CouldntGetTask Message:Pipeline .* can't be Run; it contains Tasks that don't exist: Couldn't retrieve Task .resolver type bundles.*name = .*: error requesting remote resource: etcdserver: request timed out"),
    ...
    ("Couldnt get task via git resolver because ETCD timeouted", r"PipelineRun .* in namespace .* failed: .* Reason:CouldntGetTask Message:Pipeline .* can't be Run; it contains Tasks that don't exist: Couldn't retrieve Task .resolver type git.*url = https://github.com/.*: error requesting remote resource: error updating resource request .* with data: etcdserver: request timed out"),
    ...
    ("Repo forking failed because GitHub token expired", r"Repo forking failed: Error checking repository [^ ]+: GET https://api.github.com/repos/[^ ]+: 401 Bad credentials"),
    ("Repo forking failed because GitLab CEE certificate did not verified", r"Repo forking failed: Error getting project [^ ]+: Get .https://gitlab.cee.redhat.com/[^ ]+.: tls: failed to verify certificate: x509: certificate signed by unknown authority"),
    ...
    ("Repo forking failed on gitlab.com because 401 Unauthorized", r"Repo forking failed: Error getting project [^ ]+: GET https://gitlab.com/[^ ]+: 401 .message: 401 Unauthorized."),
    ...
    ("Repo templating failed because token expired", r"Repo-templating workflow component cleanup failed: Error templating PaC files: Failed to get file [^ ]+/COMPONENT-pull-request.yaml from repo konflux-probe-test-templates revision main: error when listing file contents: GET https://api.github.com/repos/rhtap-perf-test/konflux-probe-test-templates/contents/[^ ]+/COMPONENT-pull-request.yaml.*: 401 Bad credentials"),
    ("Repo templating failed when obtaining file from github.com because 503", r"Repo-templating workflow component cleanup failed: Error templating PaC files: Failed to get file [^ ]+ from repo [^ ]+ revision [^ ]+: error when listing file contents: GET https://api.github.com/repos/[^ ]+: 503"),
...
FAILED_PLR_ERRORS = {
    ...
    ("DNF failed to download package from Koji", r".librepo. Error during transfer: Status code: 503 for https://kojipkgs.fedoraproject.org/repos/[^ ]+.rpm.*ERROR Command returned error: Failed to download packages"),
    ...
    ("Error allocating host because all available hosts failed", r"Error allocating host: all available hosts for the platform have failed"),
    ("Error allocating host because ETCD timeouted", r"Error allocating host: etcdserver: request timed out"),
    ...
    ("Failed to compile with clang", r"ERROR: [^ ]+: Compiling [^ ]+.cc .for tool. failed: .Exit 1.: clang-[0-9]+ failed: error executing CppCompile command"),
    ...
    ("Failed to prefetch dependencies due to go env error", r"ERROR PackageManagerError: Go execution failed: .go env GOWORK. failed with rc=1"),
    ("Failed to prefetch dependencies due to invalid input when fetching tags", r"Executing: git fetch --tags.*ERROR InvalidInput: 1 validation error for user input.*packages.*Value error, package path does not exist .or is not a directory.:"),
    ("Failed to prefetch dependencies due to subscription-manager failed to register because system is already registered", r"Executing: subscription-manager register --org [^ ]+ --activationkey [^ ]+ This system is already registered. Use --force to override error: Command failed after [0-9]+ tries with status 64 Subscription-manager register failed"),
    ...
    ("Failed to pull container from quay.io due to dial tcp i/o timeout", r"pinging container registry quay.io: Get ..https://quay.io/v2/..: dial tcp [^ ]+:443: i/o timeout"),
    ...
    ("Git failed to clone submodule because GitLab CEE giving 429", r"Error running git .*: exit status 1.*error: RPC failed; HTTP 429 curl 22 The requested URL returned error: 429.*fatal: clone of 'https://gitlab.cee.redhat.com/[^ ]+' into submodule path '[^ ]+' failed"),
    ("Git failed to clone submodule because GitLab CEE giving 429", r"Error running git .*: exit status 1.*error: RPC failed; HTTP 429 curl 22 The requested URL returned error: 429.*fatal: Fetched in submodule path '[^ ]+', but it did not contain [^ ]+. Direct fetching of that commit failed"),
    ("Git failed to fetch because GitLab CEE giving 429", r"Error running git .*: exit status 128.*error: RPC failed; HTTP 429 curl 22 The requested URL returned error: 429.*Error fetching git repository: failed to fetch [^ ]+: exit status 128"),
    ("Git failed to fetch because GitLab CEE giving 429", r"Error running git .*: exit status 1.*remote: Retry later.*fatal: unable to access 'https://gitlab.cee.redhat.com/.*': The requested URL returned error: 429.*Error fetching git repository: failed to fetch [^ ]+: exit status 1"),
    ("Git failed to fetch because GitLab CEE giving 429", r"Error running git .*: exit status 1.*remote: Retry later.*fatal: unable to access 'https://gitlab.cee.redhat.com/.*': The requested URL returned error: 429.*Error fetching git repository: exit status 1"),
    ("Go failed installation because it was killed", r"Run buildah build .* go install [^ ]+ subprocess exited on killed subprocess exited with status 1"),
    ("Go failed build because of incompatible version", r"go: go.mod requires go >= [^ ]+ \(running go [^ ]+; .*\) .* exit status 1"),
    ...
    ("Prefetch dependencies failed to download from rhsm-pulp.corp.redhat.com because not whole content was fetched", r"Reading RPM lockfile: [^ ]+ .* Unsuccessful download: https://rhsm-pulp.corp.redhat.com/content/[^ ]+ .* ERROR FetchError: exception_name: ClientPayloadError, details: Response payload is not completed: .ContentLengthError: 400, message='Not enough data to satisfy content length header.'."),
    ("Prefetch dependencies failed to download from download.devel.redhat.com because of timeout", r"Reading RPM lockfile: [^ ]+ .* Unsuccessful download: https://download.devel.redhat.com/[^ ]+ .* ERROR FetchError: exception_name: TimeoutError, details:  Error: FetchError: exception_name: TimeoutError,"),
    ...
    ("Script merge_catalogs.sh due to permission error", r"RUN ./merge_catalogs.sh /bin/sh: line 1: ./merge_catalogs.sh: Permission denied subprocess exited with status 126 subprocess exited with status 126"),
    ...
    ("Script opm failed because catalog is missing", r"Running opm alpha render-template basic -o yaml [^ ]+ . catalog/catalog-prod.yaml.*unable to open .catalog/[^ ]+.: open catalog/[^ ]+: no such file or directory"),
    ("Script opm failed because catalog is missing", r"Running opm alpha render-template basic -o yaml [^ ]+ --migrate-level=bundle-object-to-csv-metadata . catalog/catalog-prod.yaml.*unable to open .catalog/[^ ]+.: open catalog/[^ ]+: no such file or directory"),
    ("Script opm failed because failed to pull image from registry.redhat.io", r"Running opm alpha render-template basic -o yaml [^ ]+ . [^ ]+.*render reference .registry.redhat.io/[^ ]+.: failed to pull image .registry.redhat.io/[^ ]+.: Source image rejected: reading signature from https://registry.redhat.io/[^ ]+: received unexpected HTTP status: 500 Internal Server Error"),
    ("Script opm failed to load or rebuild cache because some package has duplicate bundle", r"level=fatal msg=.failed to load or rebuild cache: failed to rebuild cache: build package index: process package [^ ]+: package [^ ]+ has duplicate bundle [^ ]+..*Error: building at STEP .RUN /bin/opm serve .*.: exit status 1"),
...
FAILED_TR_ERRORS = {
    ...
    ("Failed to create task run pod because ISE on webhook proxy.operator.tekton.dev", r"failed to create task run pod .*: Internal error occurred: failed calling webhook \\\"proxy.operator.tekton.dev\\\": failed to call webhook: Post \\\"https://tekton-operator-proxy-webhook.openshift-pipelines.svc:443/defaulting.timeout=10s\\\": EOF. Maybe missing or invalid Task.*PodCreationFailed"),
    ...
    ("Pod creation failed because ETCD timeouted", r".message.: .failed to create task run pod [^ ]+: etcdserver: request timed out. Maybe missing or invalid Task [^ ]+., .reason.: .PodCreationFailed."),
    ...
    ("Pod creation failed because serviceaccounts not found", r".message.: .failed to create task run pod [^ ]+: translating TaskSpec to Pod: serviceaccounts [^ ]+ not found. Maybe missing or invalid Task [^ ]+., .reason.: .PodCreationFailed."),
