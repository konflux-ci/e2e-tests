...
ERRORS = {
    ...
    ("Couldnt get pipeline because ETCD timeouted", r"PipelineRun .* in namespace .* failed: .* Reason:CouldntGetPipeline Message:Error retrieving pipeline for pipelinerun .*: resolver failed to get Pipeline : error requesting remote resource: error updating resource request .* with data: etcdserver: request timed out"),
