...
ERRORS = {
    ...
    ("Couldnt get task via git resolver from gitlab.cee due to 429", r"Reason:CouldntGetTask Message:.*Couldn't retrieve Task .resolver type git.*https://gitlab.cee.redhat.com/.* error requesting remote resource: error getting .Git. .*: git fetch error: error: RPC failed; HTTP 429 curl 22 The requested URL returned error: 429 fatal: expected flush after ref listing: exit status 128"),
