...
var _ = framework.IntegrationServiceSuiteDescribe("Gitlab Status Reporting of Integration tests", Label("integration-service", "HACBS", "gitlab-status-reporting"), func() {
	...
	Describe("Gitlab with status reporting of Integration tests in the assosiated merge request", Ordered, func() {
		BeforeAll(func() {
			...
			projectID = utils.GetEnv(constants.GITLAB_PRIJECT_ID, "")
			...
			err = f.AsKubeAdmin.CommonController.Gitlab.CreateGitlabNewBranch(projectID, componentBaseBranchName, componentRevision, componentDefaultBranch) //CreateBranch("56586709", componentBaseBranchName, componentDefaultBranch)
			Expect(err).ShouldNot(HaveOccurred())
