...
func createComponentWithCustomBranch(f framework.Framework, testNamespace, applicationName, componentRepoName, componentRepoURL string, componentBaseBranchName string) (*appstudioApi.Component, string, string, string) {
	componentName := fmt.Sprintf("%s-%s", "test-component-pac", util.GenerateRandomString(6))
	pacBranchName := constants.PaCPullRequestBranchPrefix + componentName
