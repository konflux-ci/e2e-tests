...
var _ = framework.IntegrationServiceSuiteDescribe("Integration Service E2E tests", Label("integration-service", "HACBS"), func() {
	...
	Describe("the component with git source (GitHub) is created", Ordered, Label("github-webhook"), func() {
		BeforeAll(func() {
			...
			_, err = f.AsKubeAdmin.HasController.CreateComponent(applicationName, componentName, appStudioE2EApplicationsNamespace, gitSourceURL, "", "", outputContainerImage, "")
		...
		It("create a ReleasePlan and an environment", func() {
			...
			Expect(err).ShouldNot(HaveOccurred())
			env, err = f.AsKubeAdmin.IntegrationController.CreateEnvironment(appStudioE2EApplicationsNamespace)
			Expect(err).ShouldNot(HaveOccurred())
		...
		It("create an applicationSnapshot of push event", func() {
			applicationSnapshot_push, err = f.AsKubeAdmin.IntegrationController.CreateApplicationSnapshot(applicationName, appStudioE2EApplicationsNamespace, componentName)
			Expect(err).ShouldNot(HaveOccurred())
