	...
	Describe("with status reporting of Integration tests in CheckRuns", Ordered, func() {
		...
		When("Integration PipelineRuns completes successfully", func() {
			It("should lead to Snapshot CR being marked as failed", FlakeAttempts(3), func() {
				...
				}, time.Minute*5, time.Second*5).Should(BeTrue(), fmt.Sprintf("Timed out waiting for Snapshot to be marked as failed %s/%s", snapshot.GetNamespace(), snapshot.GetName()))
			})
			...
			It("snapshot creation failure is reported to integration test checkRun", func() {
				Eventually(func() error {
					...
					if strings.Contains(*text, "Failed to create snapshot") {
						GinkgoWriter.Printf("failed to get expected message about snapshot creation failulre")
