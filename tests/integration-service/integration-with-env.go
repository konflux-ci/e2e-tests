...
var _ = framework.IntegrationServiceSuiteDescribe("Integration Service E2E tests", Label("integration-service", "integration-env"), func() {
	...
	Describe("with happy path for general flow of Integration service with ephemeral environment", Ordered, func() {
		BeforeAll(func() {
			...
			integrationTestScenario, err = f.AsKubeAdmin.IntegrationController.CreateIntegrationTestScenario("", applicationName, testNamespace, gitURL, itsRevision, pathIntegrationPipelineWithEnv)
		...
		When("the build pipelineRun run succeeded", func() {
			...
			It("checks if CronJob spacerequest-cleaner exists", func() {
				...
				Expect(err).ShouldNot(HaveOccurred())
				Expect(strings.Contains(spaceRequestCleanerCronJob.Name, "spacerequest-cleaner")).Should(BeTrue())
			})
			...
			It("checks that when deleting integration test scenario pipelineRun, spaceRequest is deleted too", func() {
				...
				Eventually(func() error {
					...
					Expect(err).ToNot(HaveOccurred())
					if currentSpaceRequest != nil {
						return fmt.Errorf("spaceRequest %s still exists", spaceRequest.Name)
					}
