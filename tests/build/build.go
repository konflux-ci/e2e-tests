	...
	ginkgo.DescribeTableSubtree("test PaC component build", ginkgo.Ordered, ginkgo.Label("github-webhook", "pac-build", "pipeline", "image-controller"), func(gitProvider git.GitProvider, gitPrefix string) {
		...
		ginkgo.When("a new component without specified branch is created and with visibility private", ginkgo.Label("pac-custom-default-branch"), func() {
			...
			ginkgo.It("correctly targets the default branch (that is not named 'main') with PaC", func() {
				...
				gomega.Eventually(func() bool {
					...
					return false
				}, timeout, interval).Should(gomega.BeTrue(), fmt.Sprintf("timed out when waiting for init PaC PR to be created against %s branch in %s repository", helloWorldComponentDefaultBranch, helloWorldComponentGitSourceRepoName))
			})
			...
			ginkgo.It("PR branch should not exist in the repo", func() {
				...
				gomega.Eventually(func() (bool, error) {
					...
					return exists, nil
				}, timeout, interval).Should(gomega.BeFalse(), fmt.Sprintf("timed out when waiting for the branch %s to be deleted from %s repository", customDefaultComponentBranch, helloWorldComponentGitSourceRepoName))
			})
		...
		ginkgo.When("a new Component with specified custom branch is created", ginkgo.Label("build-custom-branch"), func() {
			...
			ginkgo.It("should lead to a PaC init PR creation", func() {
				...
				gomega.Eventually(func() bool {
					...
					return false
				}, timeout, interval).Should(gomega.BeTrue(), fmt.Sprintf("timed out when waiting for init PaC PR (branch name '%s') to be created in %s repository", pacBranchName, helloWorldComponentGitSourceRepoName))
			})
			...
			ginkgo.It("eventually leads to the PipelineRun status report at Checks tab", func() {
				...
				case git.GitHubProvider:
					...
					gomega.Expect(f.AsKubeAdmin.CommonController.Github.GetCheckRunConclusion(expectedCheckRunName, helloWorldComponentGitSourceRepoName, prHeadSha, prNumber)).To(gomega.Equal(constants.CheckrunConclusionSuccess))
				case git.GitLabProvider:
					expectedStatusName := fmt.Sprintf("%s-%s", customBranchComponentName, "on-pull-request")
					gomega.Expect(f.AsKubeAdmin.HasController.GitLab.GetCommitStatusConclusion(expectedStatusName, helloWorldComponentGitLabProjectID, prHeadSha, prNumber)).To(gomega.Equal(constants.CheckrunConclusionSuccess))
				}
			})
		})
		...
		ginkgo.When("the PaC init branch is updated", ginkgo.Label("build-custom-branch"), func() {
			...
			ginkgo.It("eventually leads to another update of a PR about the PipelineRun status report at Checks tab", func() {
				...
				case git.GitHubProvider:
					expectedCheckRunName := fmt.Sprintf("%s-%s", customBranchComponentName, "on-pull-request")
					gomega.Expect(f.AsKubeAdmin.CommonController.Github.GetCheckRunConclusion(expectedCheckRunName, helloWorldComponentGitSourceRepoName, createdFileSHA, prNumber)).To(gomega.Equal(constants.CheckrunConclusionSuccess))
				case git.GitLabProvider:
					expectedStatusName := fmt.Sprintf("%s-%s", customBranchComponentName, "on-pull-request")
					gomega.Expect(f.AsKubeAdmin.HasController.GitLab.GetCommitStatusConclusion(expectedStatusName, helloWorldComponentGitLabProjectID, createdFileSHA, prNumber)).To(gomega.Equal(constants.CheckrunConclusionSuccess))
				}
			})
		})
		...
		ginkgo.When("the PaC init branch is merged", ginkgo.Label("build-custom-branch"), func() {
			...
			ginkgo.BeforeAll(func() {
				gomega.Eventually(func() error {
					...
					return err
				}, time.Minute).ShouldNot(gomega.HaveOccurred(), fmt.Sprintf("error when merging PaC pull request #%d in repo %s", prNumber, helloWorldComponentGitSourceRepoName))
		...
		ginkgo.When("the component is removed and recreated (with the same name in the same namespace)", ginkgo.Label("build-custom-branch"), func() {
			...
			ginkgo.AfterAll(func() {
				...
				gomega.Eventually(func() bool {
					...
					return false
				}, time.Minute, time.Second*10).Should(gomega.BeTrue(), fmt.Sprintf("timed out when waiting for purge PR with traget branch %s to be created in %s repository", componentBaseBranchName, helloWorldComponentGitSourceRepoName))
	...
	ginkgo.DescribeTableSubtree("test of component update with renovate", ginkgo.Ordered, ginkgo.Label("renovate", "multi-component"), func(gitProvider git.GitProvider, gitPrefix string) {
		...
		ginkgo.AfterAll(func() {
			...
			for i, c := range components {
				...
				if err != nil {
					gomega.Expect(err.Error()).To(gomega.Or(gomega.ContainSubstring("Reference does not exist"), gomega.ContainSubstring("404 Not Found"), gomega.ContainSubstring("Branch Not Found")))
				}
		...
		ginkgo.When("components are created in same namespace", func() {
			...
			ginkgo.It(fmt.Sprintf("should lead to a nudge PR creation for child component %s", ChildComponentDef.componentName), func() {
				...
				gomega.Eventually(func() bool {
					...
					return false
				}, timeout, interval).Should(gomega.BeTrue(), fmt.Sprintf("timed out when waiting for component nudge PR to be created in %s repository", componentDependenciesChildRepoName))
			})
			ginkgo.It(fmt.Sprintf("merging the PR should be successful for child component %s", ChildComponentDef.componentName), func() {
				gomega.Eventually(func() error {
					...
					return err
				}, time.Minute).ShouldNot(gomega.HaveOccurred(), fmt.Sprintf("error when merging nudge pull request #%d in repo %s", prNumber, componentDependenciesChildRepoName))
