...
var _ = framework.BuildSuiteDescribe("Build service E2E tests", Label("build", "HACBS"), func() {
	...
	Describe("the component with git source (GitHub) is created", Ordered, Label("github-webhook"), func() {
		...
		BeforeAll(func() {
			...
			_, err = f.AsKubeAdmin.HasController.CreateComponentWithPaCEnabled(applicationName, componentName, testNamespace, helloWorldComponentGitSourceURL, outputContainerImage)
			Expect(err).ShouldNot(HaveOccurred())
	...
	Describe("HACBS pipelines", Ordered, func() {
		...
		BeforeAll(func() {
			...
			customBundleConfigMap, err := f.AsKubeAdmin.CommonController.GetConfigMap(constants.BuildPipelinesConfigMapName, testNamespace)
			if err != nil {
				if errors.IsNotFound(err) {
					...
					_, err = f.AsKubeAdmin.CommonController.CreateConfigMap(hacbsBundleConfigMap, testNamespace)
					Expect(err).ToNot(HaveOccurred())
					DeferCleanup(f.AsKubeAdmin.CommonController.DeleteConfigMap, constants.BuildPipelinesConfigMapName, testNamespace, false)
				} else {
				...
				_, err = f.AsKubeAdmin.CommonController.UpdateConfigMap(hacbsBundleConfigMap, testNamespace)
				Expect(err).ToNot(HaveOccurred())
			...
			for _, gitUrl := range componentUrls {
				...
				outputContainerImage = fmt.Sprintf("quay.io/%s/test-images:%s", utils.GetQuayIOOrganization(), strings.Replace(uuid.New().String(), "-", "", -1))
				// Create a component with Git Source URL being defined
				_, err := f.AsKubeAdmin.HasController.CreateComponent(applicationName, componentName, testNamespace, gitUrl, "", "", outputContainerImage, "")
				Expect(err).ShouldNot(HaveOccurred())
			}
		})
	...
	Describe("Creating component with container image source", Ordered, func() {
		...
		BeforeAll(func() {
			...
			_, err = f.AsKubeAdmin.CommonController.CreateTestNamespace(testNamespace)
			Expect(err).NotTo(HaveOccurred(), "Error when creating/updating '%s' namespace: %v", testNamespace, err)
			DeferCleanup(f.AsKubeAdmin.CommonController.DeleteNamespace, testNamespace)
			...
			_, err = f.AsKubeAdmin.HasController.CreateComponent(applicationName, componentName, testNamespace, "", "", containerImageSource, outputContainerImage, "")
			Expect(err).ShouldNot(HaveOccurred())
			DeferCleanup(f.AsKubeAdmin.TektonController.DeleteAllPipelineRunsInASpecificNamespace, testNamespace)
		})
	...
	Describe("Creating a configmap with 'dummy' custom pipeline bundle in the testing namespace", Ordered, func() {
		...
		BeforeAll(func() {
			...
			_, err = f.AsKubeAdmin.CommonController.CreateConfigMap(cm, testNamespace)
			Expect(err).ToNot(HaveOccurred())

			componentName = "build-suite-test-bundle-overriding"
			outputContainerImage := fmt.Sprintf("quay.io/%s/test-images:%s", utils.GetQuayIOOrganization(), strings.Replace(uuid.New().String(), "-", "", -1))
			_, err = f.AsKubeAdmin.HasController.CreateComponent(applicationName, componentName, testNamespace, helloWorldComponentGitSourceURL, "", "", outputContainerImage, "")
			Expect(err).ShouldNot(HaveOccurred())
			DeferCleanup(f.AsKubeAdmin.TektonController.DeleteAllPipelineRunsInASpecificNamespace, testNamespace)
		...
		// AfterAll(func() {
		// 	f.AsKubeAdmin.CommonController.DeleteNamespace(testNamespace)
		// })
	...
	Describe("Creating a component with a specific container image URL", Ordered, func() {
		...
		AfterAll(func() {
			Expect(f.AsKubeAdmin.HasController.DeleteAllComponentsInASpecificNamespace(testNamespace, 30*time.Second)).To(Succeed())
			Expect(f.AsKubeAdmin.HasController.DeleteAllApplicationsInASpecificNamespace(testNamespace, 30*time.Second)).To(Succeed())
			Expect(f.AsKubeAdmin.TektonController.DeleteAllPipelineRunsInASpecificNamespace(testNamespace)).To(Succeed())
			Expect(f.AsKubeAdmin.CommonController.DeleteNamespace(testNamespace)).To(Succeed())
		})
