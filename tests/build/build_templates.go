...
func CreateComponent(commonCtrl *common.SuiteController, ctrl *has.HasController, applicationName, componentName, namespace string, scenario ComponentScenarioSpec) {
	...
	if scenario.CheckDockerMediaType {
...
var _ = framework.BuildSuiteDescribe("Build templates E2E test", Label("build", "build-templates", "HACBS"), func() {
	...
	Describe("HACBS pipelines", Ordered, Label("pipeline"), func() {
		...
		for componentName, scenario := range components {
			...
			It("image mediaType is correct", Label(buildTemplatesTestLabel), func() {
				builtImage := build.GetBinaryImage(pr)
				fmt.Println(builtImage)
				if scenario.CheckDockerMediaType {
					// Check for docker mediaType
					Expect(build.ValidateMediaTypeInImage(builtImage, "docker")).To(BeTrue(), "mediaType in the image is not of type docker")
				} else {
					// Check for oci mediaType
					Expect(build.ValidateMediaTypeInImage(builtImage, "oci")).To(BeTrue(), "mediaType in the image is not of type oci")
				}
			})
