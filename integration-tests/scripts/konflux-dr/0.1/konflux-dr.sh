...
post_actions() {
    ...
    if [[ "$exit_code" == "124" ]]; then
        ...
        printf "\n\n" | tee -a "${ARTIFACT_DIR}/dr-tests.log"
        log "ERROR" "The process for bootstrapping the cluster and running tests timed out after $DR_TIMEOUT" | tee -a "${ARTIFACT_DIR}/dr-tests.log"
    fi
...
timeout "$DR_TIMEOUT" make ci/test/disaster-recovery 2>&1 | tee "${ARTIFACT_DIR}/dr-tests.log"
