...
func setup(cmd *cobra.Command, args []string) {
	...
	if len(token) == 0 {
		...
		if err != nil {
			tokenRequestURI, err := auth.GetTokenRequestURI(framework.AsKubeAdmin.CommonController.KubeRest()) // authorization.FindTokenRequestURI(framework.AsKubeAdmin.CommonController.KubeRest())
			if err != nil {
