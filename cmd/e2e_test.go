...
var _ = ginkgo.SynchronizedBeforeSuite(func() []byte {
	...
	gomega.Expect(err).ShouldNot(gomega.HaveOccurred(), "Error when trying to start a new K8S client")
	...
	gomega.Expect(err).ShouldNot(gomega.HaveOccurred(), "Error when trying to retrieve secret information")
