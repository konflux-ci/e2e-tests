...
#DOCKERCONFIGJSON=
DOCKERCONFIGJSON=
...
function prepareAndInstallACM() {
    ...
    git clone https://github.com/stolostron/deploy.git "$WORKSPACE"/singapore_tmp/deploy
...
type: kubernetes.io/dockerconfigjson" > "$WORKSPACE"/singapore_tmp/deploy/prereqs/pull-secret.yaml

    # configure snapshot.ver with the SNAPSHOT tag to install in silent mode
    echo "$SNAPSHOT_VER" > "$WORKSPACE"/singapore_tmp/deploy/snapshot.ver
    echo "snapshot.ver set to $SNAPSHOT_VER"

    # Install Open Cluster Management
    cd "$WORKSPACE"/singapore_tmp/deploy 
