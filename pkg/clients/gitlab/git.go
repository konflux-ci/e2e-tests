...
func (gc *GitlabClient) CloseMergeRequest(projrctID string, mergeRequestIID int) error {

	// Get merge requests using Gitlab client
	_, _, err := gc.client.MergeRequests.GetMergeRequest(projrctID, mergeRequestIID, nil)
	if err != nil {
		return fmt.Errorf("failed to get MR of IID %d in projectID %s, %v", mergeRequestIID, projrctID, err)
	}

	_, _, err = gc.client.MergeRequests.UpdateMergeRequest(projrctID, mergeRequestIID, &gitlab.UpdateMergeRequestOptions{
		StateEvent: gitlab.String("close"),
	})
	if err != nil {
		return fmt.Errorf("failed to close MR of IID %d in projectID %s, %v", mergeRequestIID, projrctID, err)
	}
